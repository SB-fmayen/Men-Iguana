...
import { buildSearchDataSource } from '@/repositories/menu-repository';
...
function SearchResults() {
  ...
  const dataSource = useMemo(() => {
    const firestoreDataSource = buildSearchDataSource(categories, items);

    if (firestoreDataSource.length === 0) {
  ...
  const searchResults = dataSource
    .map((categoryGroup) => ({
      ...
      items: categoryGroup.items.filter((item) =>
        ...
        categoryGroup.category.toLowerCase().includes(searchText.toLowerCase())
      ),
    }))
          ...
          <main className="container mx-auto px-4 py-12">
            ...
            <div className="mb-12">
              ...
              <p className="text-lg text-gray-600">
                {totalResults > 0 ? (
                  <>
                    Se encontraron <span className="font-bold text-orange-600">{totalResults}</span> resultados para "{searchText}"
                  </>
                ) : (
